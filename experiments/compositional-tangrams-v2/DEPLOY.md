...
python3 scripts/mark_paid.py full_sample/2026-09-04-20-32-20
```
...
**4a. [L]** Regenerate the derived timeout screen. A player who let the per-item
clock expire on >75% of their descriptions was not at the keyboard while it ran;
their dyad is dropped and the slot re-opened. This file is DERIVED — regenerate
...
`completedStudy` true, and that test is wrong in both directions:

- `--exclude-games` — a dyad excluded post hoc (AI use, degenerate responses,
  timed-out phases) completed exactly like a real one, so without this its set
  reads as finished and the hole is only discovered at analysis time, after
  recruitment closes. Two files: `excluded_games.csv` is hand-adjudicated,
  `timeout_games.csv` is regenerated by step 4a.
- `--include-games` — `completedStudy` is set on the final exit step, so a dyad
...
Re-run with `--write` once the report looks right. Pass the same lists — the
schedule is derived from coverage, so dropping them here writes a different
